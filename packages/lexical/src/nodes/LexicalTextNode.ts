...
import type { EditorConfig, KlassConstructor, LexicalEditor, Spread, TextNodeThemeClasses } from '../LexicalEditor'
import type {
	DOMConversionMap,
	DOMConversionOutput,
	DOMExportOutput,
...
export class TextNode extends LexicalNode {
	...
	static importJSON(serializedNode: SerializedTextNode, update?: boolean): TextNode {
		const node = $createTextNode(serializedNode.text, serializedNode.key)
...
export function $createTextNode(text = '', key?: string): TextNode {
	return $applyNodeReplacement(new TextNode(text, key))
}
