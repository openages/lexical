			...
			if ($isRangeSelection(selection)) {
				...
				if (
					...
					const shouldLetBrowserHandleDelete =
						IS_ANDROID_CHROME && isSelectionAnchorSameAsFocus && !hasSelectedAllTextInNode
				...
				: (event: Event) => {
						...
						switch (eventName) {
							...
							case 'paste':
								return (
									isEditable &&
									dispatchCommand(editor, PASTE_COMMAND, event as ClipboardEvent)
								)
...
export function removeRootElementEvents(rootElement: HTMLElement): void {
	const doc = rootElement.ownerDocument
	const documentRootElementsCount = rootElementsRegistered.get(doc)
	invariant(documentRootElementsCount !== undefined, 'Root element not registered')
