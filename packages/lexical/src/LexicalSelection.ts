...
export class RangeSelection implements BaseSelection {
	...
	insertNodes(nodes: Array<LexicalNode>): void {
		...
		if (!nodes.some(notInline)) {
			invariant($isElementNode(firstBlock), "Expected 'firstBlock' to be an ElementNode")
			const index = $removeTextAndSplitBlock(this)
			firstBlock.splice(index, 0, nodes)
			last.selectEnd()
			return
	...
	deleteCharacter(isBackward: boolean): void {
		...
		if (this.isCollapsed()) {
			...
			if (!this.isCollapsed()) {
				...
				const element =
					anchor.type === 'element' ? anchor.getNode() : anchor.getNode().getParentOrThrow()
...
export function $internalCreateSelection(editor: LexicalEditor): null | BaseSelection {
	const currentEditorState = editor.getEditorState()
	const lastSelection = currentEditorState._selection
	const domSelection = getDOMSelection(editor._window)
...
function $splitNodeAtPoint(node: LexicalNode, offset: number): [parent: ElementNode, offset: number] {
	const parent = node.getParent()
	if (!parent) {
		const paragraph = $createParagraphNode()
