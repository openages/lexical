...
function $updateParagraphNodeProperties<T extends ParagraphNode>(target: T, source: ParagraphNode): T {
 ...
 */
export function $cloneWithProperties<T extends LexicalNode>(node: T, new_key?: boolean): T {
	const constructor = node.constructor
	// @ts-expect-error
	const clone: T = constructor.clone(node, new_key)
 ...
 */
export function $sliceSelectedTextNodeContent(selection: BaseSelection, textNode: TextNode): LexicalNode {
...
export function $isAtNodeEnd(point: Point): boolean {
	...
	invariant($isElementNode(node), 'isAtNodeEnd: node must be a TextNode or ElementNode')
...
export function $trimTextContentFromAnchor(editor: LexicalEditor, anchor: Point, delCount: number): void {
	...
	while (remaining > 0 && currentNode !== null) {
		if ($isElementNode(currentNode)) {
			const lastDescendant: null | LexicalNode = currentNode.getLastDescendant<LexicalNode>()
...
function $patchStyle(
	...
	patch: Record<string, string | null | ((currentStyleValue: string | null) => string)>
): void {
